...
class SuiteDeCouchePdf {
	...
	protected function getNoteTC() {
		...
		$this->_page->drawText ( 'SUITE DE COUCHES', $this->_leftMargin + 160, $this->_yPosition );
		$this->_yPosition -= 5;
		...
		$tab = $this->scinderText ( $this->_DonneesDemande ['suite_de_couches'] );
		...
		for($i = 1; $i < 18; $i ++) {
			...
			if ($i == 1) {
				...
				$this->_page->drawText ( '', $this->_leftMargin, $this->_yPosition );
				$this->_page->setFont ( $this->_policeContenu, 12 );
				$this->_page->drawText ( iconv ( 'UTF-8', 'ISO-8859-1', $tab [1] ), $this->_leftMargin , $this->_yPosition );
			} else if ($i == 2) {
				$this->_page->setFont ( $this->_policeContenu, 12 );
