...
class DemandeExamenPdf {
	...
	public function setEnTete() {
		...
		$this->_page->drawText ( 'Demande d\'examen Bio & Morpho', $this->_leftMargin + 155, $this->_yPosition );
		$this->_yPosition -= 5;
	...
	protected function getNoteBio() {
		...
		while ( $this->_yPosition > 70 ) {
			if ($this->_DonneesDemande && $i == 1) {
				...
				$this->_page->drawText ( iconv ( 'UTF-8', 'ISO-8859-1', 'Examens Biologiques:' ), $this->_leftMargin, $this->_yPosition );
				
				$this->_page->setlineColor ( new ZendPdf\Color\Html ( 'gray' ) );
				// $this->_page->setLineWidth(0.7);
				$this->_page->drawLine ( $this->_leftMargin, $this->_yPosition - 5, $this->_pageWidth - $this->_leftMargin - 370, $this->_yPosition - 5 );
			...
			if ($i <= $examensBio) {
				...
				if ($this->_DonneesDemande [$i]) {
					...
					$this->_page->drawText ( $i . ') ' . iconv ( 'UTF-8', 'ISO-8859-1', $this->listeDesExamensBioMorpho ()[$this->_DonneesDemande [$i]] . ':' ), $this->_leftMargin, $this->_yPosition );
					$this->_page->setFont ( $this->_policeContenu, 12 );
					$this->_page->drawText ( iconv ( 'UTF-8', 'ISO-8859-1', $this->_NotesDemande [$i] ), $this->_leftMargin + 138, $this->_yPosition );
				}
			}
			...
			if ($this->_DonneesDemandeMorph && $i == count ( $this->_DonneesDemande ) + 1) {
				...
				$this->_page->setFont ( $this->_courier, 15 );
				$this->_page->drawText ( iconv ( 'UTF-8', 'ISO-8859-1', 'Examens Morphologiques:' ), $this->_leftMargin, $this->_yPosition );
				
				$this->_page->setlineColor ( new ZendPdf\Color\Html ( 'gray' ) );
				// $this->_page->setLineWidth(0.7);
				// $this->_page->setLineDashingPattern(array(0, 0));
				$this->_page->drawLine ( $this->_leftMargin, $this->_yPosition - 5, $this->_pageWidth - $this->_leftMargin - 345, $this->_yPosition - 5 );
			...
			if ($i > $examensBio && $i <= ($examensBio + $examensMorpho)) {
				...
				if ($this->_DonneesDemandeMorph [$i]) {
					...
					$this->_page->setFont ( $this->_newTimeGras, 12 );
					$this->_page->drawText ( $n ++ . ') ' . iconv ( 'UTF-8', 'ISO-8859-1', $this->listeDesExamensBioMorpho ()[$this->_DonneesDemandeMorph [$i]] . ':' ), $this->_leftMargin, $this->_yPosition );
					$this->_page->setFont ( $this->_policeContenu, 12 );
					$this->_page->drawText ( iconv ( 'UTF-8', 'ISO-8859-1', $this->_notesDemandeMorph [$i] ), $this->_leftMargin + 138, $this->_yPosition );
				}
			}
			...
			$this->_page->drawLine ( $this->_leftMargin, $this->_yPosition - 5, $this->_pageWidth - $this->_leftMargin, $this->_yPosition - 5 );
			$this->_yPosition -= $noteLineHeight;
		}
