...
class DemandeTable {
	...
	public function resultatExamens($id_cons) {
		if ($this->getDemande ( $id_cons )) {
			...
			$sQuery = $sql->select ()->from ( array (
					...
					'result' => 'resultats_examens' 
			), 'result.idDemande = d.idDemande', array (
					'*' 
			) )->join ( array (
					'resul_Img' => 'resultats_image' 
			), 'resul_Img.idResultat = result.idResultat', array (
					'NomImage' => 'nomImage' 
			) )->where ( array (
					'd.idCons' => $id_cons,
					'result.envoyer' => 1 
	...
	public function ajouterImageMorpho($id_cons, $idExamen, $nomImage, $dateEnregistrement, $id_personne) {
		...
		if ($demande) {
			...
			if ($resultat) {
				...
				$db = $this->tableGateway->getAdapter ();
				$sql = new Sql ( $db );
				$sQuery = $sql->update ( 'resultats_examens' )->set ( array (
						'date_modifcation' => $dateEnregistrement 
				) )->where ( array (
						'idResultat' => $resultat ['idResultat'] 
				) );
				$stat = $sql->prepareStatementForSqlObject ( $sQuery );
				$stat->execute ();
				...
				$db = $this->tableGateway->getAdapter ();
				$sql = new Sql ( $db );
				$sQuery = $sql->insert ()->into ( 'resultats_image' )->columns ( array (
						'nomImage',
						'dateEnregistrement',
						'idResultat' 
				) )->values ( array (
				...
				$db = $this->tableGateway->getAdapter ();
				$sql = new Sql ( $db );
				$sQuery = $sql->insert ()->into ( 'resultats_examens' )->columns ( array (
						'idDemande',
						'date_enregistrement',
						'id_personne' 
				) )->values ( array (
	...
	public function verifierResultatExiste($idDemande) {
		...
		$select->from ( array (
				're' => 'resultats_examens' 
		) );
		$select->where ( array (
				're.idDemande' => $idDemande 
		) );
		$stat = $sql->prepareStatementForSqlObject ( $select );
		$result = $stat->execute ()->current ();
		return $result;
	...
	public function recupererDonneesExamen($id_cons, $id, $typeExamen) {
		if ($this->getDemande ( $id_cons )->current ()) {
			...
			$donnees = array (
					'IdImage' => $tabIdImage [$id],
					'NomImage' => $tabNomImage [$id] 
			);
			return $donnees;
		}
	}
	...
	public function supprimerImage($idImage) {
		...
		$db = $this->tableGateway->getAdapter ();
		$sql = new Sql ( $db );
		$sQuery = $sql->delete ( 'resultats_image' )->where ( array (
				'idImage' => $idImage 
		) );
		$stat = $sql->prepareStatementForSqlObject ( $sQuery );
		$Result = $stat->execute ();
	...
	public function supprimerResultat($idResultat) {
		...
		$db = $this->tableGateway->getAdapter ();
		$sql = new Sql ( $db );
		$sQuery = $sql->delete ( 'resultats_examens' )->where ( array (
				'idResultat' => $idResultat 
		) );
		$stat = $sql->prepareStatementForSqlObject ( $sQuery );
		$Result = $stat->execute ();
				...
				if (! in_array ( $listeDemande ['idExamen'], $examens )) {
					if ($listeDemande ['idExamen'] > 7) {
						...
						if ($resultat) {
							...
							$this->tableGateway->delete ( array (
									'idDemande' => $listeDemande ['idDemande'] 
							) );
						} else {
							$this->tableGateway->delete ( array (
									'idDemande' => $listeDemande ['idDemande'] 
