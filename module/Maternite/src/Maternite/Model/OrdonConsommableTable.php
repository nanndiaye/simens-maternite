...
class OrdonConsommableTable {
	...
	public function addMedicaments($medicament) {
		...
		$sql = new Sql ( $db );
		$sQuery = $sql->insert ()->into ( 'consommable' )->values ( array (
				'INTITULE' => $medicament 
		) );
	...
	public function addFormes($libelleForme) {
		if ($this->existeFormes ( $libelleForme ) == false) {
			$db = $this->tableGateway->getAdapter ();
			$sql = new Sql ( $db );
			$sQuery = $sql->insert ()->into ( 'forme_medicament' )->values ( array (
	...
	public function existeQuantites($libelleQuantite) {
		...
		$select->from ( array (
				'qm' => 'quantite_medicament' 
		) );
		$select->where ( array (
				'qm.libelleQuantite' => $libelleQuantite 
		) );
	...
	public function addQuantites($libelleQuantite) {
		if ($this->existeQuantites ( $libelleQuantite ) == false) {
			$db = $this->tableGateway->getAdapter ();
			$sql = new Sql ( $db );
			$sQuery = $sql->insert ()->into ( 'quantite_medicament' )->values ( array (
					'libelleQuantite' => $libelleQuantite 
			) );
			$sql->prepareStatementForSqlObject ( $sQuery )->execute ();
		}
	}
